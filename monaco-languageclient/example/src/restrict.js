export default function restrictMonacoRanges(model, ranges, rangeConstructor) {
  try {
    ...
    restrictions.forEach(function (restriction, index) {
      ...
      console.log("Range ", index, " here is ", range);
      ...
      positions.some(function (position) {
        ...
        for (let i = 0; i < length; i++) {
          ...
          if (
            ...
            (range.endLineNumber === posLineNumber &&
              range.endColumn === posCol)
          ) {
    ...
    model._restrictionChangeListener = model.onDidChangeContent(function (
      contentChangedEvent
    ) {
      ...
      if (!(isUndoing && model.editInReadOnlyArea)) {
        ...
        const getInfoFrom = function (change, editableRange) {
          ...
          info.startColumnOfRange =
            range.startColumn === editableRange.startColumn;

          info.endLineOfRange =
            range.endLineNumber === editableRange.endLineNumber;
          info.endColumnOfRange = range.endColumn === editableRange.endColumn;

          info.middleLineOfRange =
            !info.startLineOfRange && !info.endLineOfRange;
